...
class StableDiffusionImgAdapter(BaseESDAdapter):
    family = "sd_img"
    component_attr = "unet"
    default_base_model_id = "CompVis/stable-diffusion-v1-4"
    default_save_path = "trained-models/sd-img/"
    ...
    def prepare_context(self, pipe, config: ESDConfig):
        ...
        with torch.no_grad():
            ...
            prompt_embeds = prompt_embeds.to(config.device)
    ...
    def training_step(
        ...
        #
        # EXPECTED EXTRA ITEMS IN CONTEXT:
        #
        # context["pixel_values"]
        #
        # shape:
        # [B,3,H,W]
        #

        pixel_values = context["pixel_values"].to(
            device=config.device,
...
ADAPTERS = {
    ...
    "sd_img": StableDiffusionImgAdapter(),
}
...
def run_img_training(
    ...
    config.train_method = adapter.normalize_train_method(
        config.train_method
    )
    ...
    if (
        ...
        and hasattr(component, "enable_gradient_checkpointing")
    ):
        component.enable_gradient_checkpointing()

    #
    # Prepare trainable params
    #

    prepared = adapter.create_prepared_component(
        pipe,
        config.train_method,
        config,
    )
    ...
    pbar = tqdm(
        ...
        desc=f"Training IMG ({adapter.family})",
    )
    ...
    while global_step < config.iterations:
        ...
        for batch in dataloader:
            ...
            if global_step >= config.iterations:
                break
            ...
            step_result = adapter.training_step(
                pipe,
                prepared,
                context,
                config,
            ...
            loss = F.mse_loss(
                step_result.model_pred.float(),
                step_result.target.float(),
            )
            ...
            postfix = {
                "loss": f"{loss.item():.4f}",
                "timestep": step_result.timestep_index,
            }
    ...
    save_esd_checkpoint(
        prepared.state_dict(),
        checkpoint_path,
        metadata=adapter.build_metadata(config),
    )
